...
public class LineHttpCredentialVerifier implements LineCredentialVerifier {
    ...
    @Override public VerifiedLineCredential verify(String idToken) {
        ...
        try {
            @SuppressWarnings("unchecked") var body = client.post().contentType(MediaType.APPLICATION_FORM_URLENCODED).body("id_token=" + java.net.URLEncoder.encode(idToken, java.nio.charset.StandardCharsets.UTF_8) + "&client_id=" + java.net.URLEncoder.encode(channelId, java.nio.charset.StandardCharsets.UTF_8)).retrieve().body(Map.class);
            var exp = Long.parseLong(String.valueOf(body == null ? 0 : body.getOrDefault("exp", 0)));
            if (body == null || body.get("sub") == null || exp <= Instant.now().getEpochSecond() || !channelId.equals(String.valueOf(body.get("aud"))) || !"https://access.line.me".equals(body.get("iss"))) throw new LineCredentialInvalidException("Invalid LINE credential");
