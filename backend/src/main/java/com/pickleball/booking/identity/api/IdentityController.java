...
public class IdentityController {
    ...
    @PostMapping("/auth/line/login") public ApiResponse<LoginResponse> login(@Valid @RequestBody LineLoginRequest request, HttpServletRequest http) { var result = service.login(request.idToken(), request.accessToken()); return ok(new LoginResponse(result.accessToken(), "Bearer", result.expiresIn(), new LoginUser(result.userId(), result.displayName(), result.roles())), http); }
    @PostMapping("/auth/line/admin/exchange") public ApiResponse<LoginResponse> adminExchange(@Valid @RequestBody AdminLineExchangeRequest request, HttpServletRequest http) { var result = service.adminLogin(request.authorizationCode(), request.codeVerifier(), request.nonce(), request.redirectUri()); return ok(new LoginResponse(result.accessToken(), "Bearer", result.expiresIn(), new LoginUser(result.userId(), result.displayName(), result.roles())), http); }
    ...
    public record LineLoginRequest(@Size(max = 10000) String idToken, @Size(max = 10000) String accessToken) {
        @AssertTrue(message = "Exactly one LINE credential is required") public boolean hasExactlyOneCredential() { return (idToken != null && !idToken.isBlank()) != (accessToken != null && !accessToken.isBlank()); }
    }
