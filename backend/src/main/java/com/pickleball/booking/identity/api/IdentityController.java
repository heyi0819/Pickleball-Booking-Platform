...
public class IdentityController {
    ...
    @PatchMapping("/me/profile") public ApiResponse<IdentityService.MeView> profile(Authentication authentication, @Valid @RequestBody ProfileRequest request, HttpServletRequest http) { return ok(service.updateProfile(principal(authentication), new UserProfile(request.displayName(), request.phone(), request.email(), request.locale())), http); }
    ...
    public record ProfileRequest(@NotBlank @Size(max = 100) String displayName, @Size(max = 30) String phone, @Email @Size(max = 254) String email, @NotBlank @Size(max = 10) String locale) {}
