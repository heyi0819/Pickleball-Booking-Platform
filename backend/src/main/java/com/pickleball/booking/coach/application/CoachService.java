...
public class CoachService {
    ...
    @Transactional public CoachApplicationEntity reviewApplication(AuthenticatedPrincipal actor, UUID id, boolean approve, String note) {
        var app=applications.findLockedById(id).orElseThrow(()->new BusinessException("RESOURCE_NOT_FOUND","Coach application was not found")); requireCommittee(actor,app.getOrganizationId()); var profile=profiles.findById(app.getCoachProfileId()).orElseThrow(()->new BusinessException("RESOURCE_NOT_FOUND","Coach profile was not found")); if (actor.userId().equals(profile.getUserId())) throw new BusinessException("REVIEWER_SELF_APPROVAL_FORBIDDEN","A reviewer cannot review their own coach application");
    ...
    @Transactional public AvailabilityProposalEntity reviewAvailability(AuthenticatedPrincipal actor, UUID id, boolean approve, String note) { var proposal=availability.findById(id).orElseThrow(()->new BusinessException("RESOURCE_NOT_FOUND","Availability proposal was not found"));requireCommittee(actor,proposal.getOrganizationId()); profiles.findById(proposal.getCoachProfileId()).orElseThrow(()->new BusinessException("RESOURCE_NOT_FOUND","Coach profile was not found")); proposal.review(approve,actor.userId(),note);audit.record(proposal.getOrganizationId(),actor.userId(),approve?"AVAILABILITY_APPROVED":"AVAILABILITY_REJECTED","AvailabilityProposal",proposal.getId(),note);return proposal; }
