...
class RefundConcurrencyIT {
    ...
    @Test
    void concurrentRefundRequestsReservePaymentCapacitySerially() throws Exception {
        ...
        try (var executor = Executors.newFixedThreadPool(2)) {
            Callable<String> first = attempt(
                    ready, start, actor, fixture, "refund-concurrent-a-" + compact(fixture.receivableId()));
            Callable<String> second = attempt(
                    ready, start, actor, fixture, "refund-concurrent-b-" + compact(fixture.receivableId()));
            ...
            assertThat(List.of(firstResult.get(), secondResult.get()))
                    .containsExactlyInAnyOrder("SUCCESS", "REFUND_EXCEEDS_REFUNDABLE");
        }

        assertThat(jdbc.queryForObject("""
                select count(*) from refunds
                where payment_id = ? and status in ('PENDING_APPROVAL','APPROVED','COMPLETED')
                """, Integer.class, fixture.paymentId())).isEqualTo(1);
        assertThat(jdbc.queryForObject("""
                select coalesce(sum(amount), 0) from refunds
                where payment_id = ? and status in ('PENDING_APPROVAL','APPROVED','COMPLETED')
                """, BigDecimal.class, fixture.paymentId())).isEqualByComparingTo("800.00");
    }
            ...
            try {
                refundService.requestRefund(
                        ...
                        new RefundApplicationService.RequestRefundCommand(
                                fixture.paymentId(), new BigDecimal("800.00"), "concurrent refund"),
                        idempotencyKey,
    ...
    private Fixture seedPaidFixture() {
        ...
        jdbc.update("insert into organizations(id, code, name) values (?, ?, ?)",
                organizationId, "s6rc-" + compact(organizationId), "Slice 6 refund concurrency");
        ...
        jdbc.update("""
                ...
                """, courseId, organizationId, "S6RC-" + compact(courseId), committeeUserId);
        jdbc.update("""
                insert into course_sessions(
                    id, organization_id, course_id, sequence_no,
                    scheduled_start_at, scheduled_end_at,
                    expected_participant_count, guest_participant_count, status)
                values (?, ?, ?, 1, '2030-08-15 10:00+00', '2030-08-15 11:00+00', 1, 0, 'SCHEDULED')
        ...
        var payment = receivableService.recordPayment(
                ...
                new ReceivableApplicationService.RecordPaymentCommand(
                        new BigDecimal("1200.00"), PaymentMethod.CASH,
                        Instant.now().minusSeconds(1), payerUserId, "paid before concurrent refunds"),
                "payment-before-refund-concurrency-" + compact(receivableId),
                "payment-before-refund-concurrency");
        return new Fixture(committeeUserId, receivableId, payment.paymentId());
    }
    ...
    private record Fixture(UUID committeeUserId, UUID receivableId, UUID paymentId) {}
}
