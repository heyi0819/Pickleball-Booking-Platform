...
test("committee Admin reviews a reschedule request and directly reschedules a formal session", async ({ page }) => {
  ...
  const operations = page.getByRole("region", { name: "課程營運" });
  await expect(operations.getByRole("heading", { name: "課程營運" })).toBeVisible();
  await expect(operations).toContainText("School event");

  const changeRow = operations.getByRole("listitem").filter({ hasText: "School event" });
  await changeRow.getByRole("button", { name: "審核通過" }).click();
  const reviewDialog = page.getByRole("dialog", { name: "確認課程營運審核" });
  await reviewDialog.getByLabel("審核原因").fill("Admin review passed");
  await reviewDialog.getByRole("button", { name: "確認審核" }).click();
  await expect(operations.getByRole("status")).toContainText("審核結果已儲存");
  await expect(operations).toContainText("目前沒有待審核的改期申請。");
  ...
  await operations.getByRole("button", { name: "查看課程" }).click();
  const courseSessions = page.getByRole("region", { name: "正式課程場次" });
  await expect(courseSessions).toContainText(course.courseNo);
  await courseSessions.getByLabel("開始時間").fill("2026-09-05T10:00");
  await courseSessions.getByLabel("結束時間").fill("2026-09-05T11:00");
  await courseSessions.getByLabel("原因").fill("Court maintenance");
  await courseSessions.getByRole("button", { name: "直接改期" }).click();
  const directDialog = page.getByRole("dialog", { name: "確認直接改期" });
  await expect(directDialog).toContainText("Court maintenance");
  await directDialog.getByRole("button", { name: "確認改期" }).click();
  await expect(operations.getByRole("status")).toContainText("課程場次已重新安排，預約已同步更新。");
  expect(directBody.reason).toBe("Court maintenance");
  expect(directIdempotencyKey).toContain(`admin-direct-reschedule-${sessionId}-`);
  await expect(courseSessions).toContainText("已延期");
});
